...
export const LoginPage: React.FC = () => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    // Basic check for password, even in mock login
    if (!password) { 
    ...
    if (username === 'agent') {
      user = { username: 'agent_user', nomComplet: 'Agent Accueil', role: 'AGENT' };
      targetPath = '/agent';
    } else if (username === 'admin') {
      user = { username: 'admin_user', nomComplet: 'Administrateur Système', role: 'ADMIN' };
      targetPath = '/admin';
    } else if (username === 'directeur') {
      user = { username: 'dir_user', nomComplet: 'Directeur Général', role: 'DIRECTEUR' };
      targetPath = '/directeur';
    } else if (username.startsWith('f_')) { 
      user = { username: username, nomComplet: username, role: 'FONCTIONNAIRE' }; 
      targetPath = '/fonctionnaire';
    } else if (username.startsWith('chef_')) {
      user = { username: username, nomComplet: username, role: 'RESPONSABLE' };
      targetPath = '/responsable';
    }
    // Removed specific 'staff' login to rely on pattern matching for FONCTIONNAIRE

    if (user) { 
      setAuth(user, 'mock-token');
      navigate(targetPath);
    } else {
      // More specific alert if user is null and password was provided
      alert(`Login failed: Username '${username}' did not match any recognized patterns. Please use valid test credentials (agent, admin, directeur, f_xxxx, chef_xxxx).`);
    }
  };
      ...
      <div className="flex flex-col md:flex-row bg-white rounded-lg shadow-lg overflow-hidden max-w-5xl w-full border border-gray-200 min-h-[500px]">
        ...
        <div className="w-full md:w-1/2 p-10 flex flex-col items-center justify-center bg-white">
          ...
          <h2 className="text-[#0f172a] font-bold text-center text-lg max-w-xs leading-tight mt-4">
            Bienvenue dans lapplica tion de gestion des adhérents de la Fondation Hassan 2
          </h2>
        </div>
      </div>
    </div>
