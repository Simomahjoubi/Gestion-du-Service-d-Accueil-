...
import {
  ...
  AlertCircle, CalendarCheck, LogOut,
} from 'lucide-react';
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="p-6 space-y-5">
            {/* Si c'est un conjoint/enfant, afficher le lien vers l'adhérent principal */}
            {v?.type !== 'ADHERENT' && adh && (
            ...
            {/* Info professionnelle (adhérent uniquement) */}
                      ...
                      <div key={m.id} className="flex items-center gap-3 bg-gray-50 rounded-xl px-4 py-3 border border-gray-100">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 text-[10px] text-gray-400">
                            ...
                            {m.sexe && <span>{m.sexe}</span>}
                          </div>
                        </div>
                        {m.telephone && (
                          <span className="text-xs text-gray-400 hidden sm:block">{m.telephone}</span>
                        )}
                      </div>
                    ))}
                  </div>
...
export const FonctionnaireDashboard: React.FC = () => {
  ...
  const fonctId = user?.id ?? 1;

  const [dossierVisiteurId, setDossierVisiteurId] = useState<number | null>(null);
  const [actionLoading, setActionLoading]         = useState(false);

  // Active visits (EN_ATTENTE + EN_COURS)
  const { data: activeVisits = [], isLoading: loadingActive } = useQuery<Visite[]>({
    queryKey: ['fonctionnaire-active', fonctId],
    queryFn: () => visiteService.getVisitesActiveByFonctionnaire(fonctId),
    refetchInterval: 20000,
  });

  // Today's visits history
  const { data: todayVisits = [], isLoading: loadingToday } = useQuery<Visite[]>({
    queryKey: ['fonctionnaire-today', fonctId],
    queryFn: () => visiteService.getVisitesTodayByFonctionnaire(fonctId),
    refetchInterval: 30000,
  });

  const refetch = () => {
    qc.invalidateQueries({ queryKey: ['fonctionnaire-active', fonctId] });
    qc.invalidateQueries({ queryKey: ['fonctionnaire-today',  fonctId] });
  };
  ...
  return (
    <div className="p-6 min-h-screen bg-[#f8f9fa]">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-gray-800">Espace de travail</h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={refetch}
            className="w-9 h-9 flex items-center justify-center border-2 border-gray-200 bg-white rounded-xl text-gray-400 hover:text-blue-600 hover:border-blue-300 transition-colors">
      ...
      <div className="flex gap-6">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <h2 className="text-sm font-bold text-gray-700 uppercase tracking-wide">File d'attente active</h2>
          ...
          {loadingActive ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 bg-white rounded-2xl border-2 border-dashed border-gray-200 text-gray-400">
              ...
              <p className="text-sm mt-1">Aucun visiteur en attente ou en cours.</p>
            </div>
          ) : (
            <>
              {/* EN_ATTENTE */}
          ...
          <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
            {loadingToday ? (
              ...
              <div className="flex flex-col items-center justify-center py-12 text-gray-400">
                ...
                <p className="text-sm">Aucune visite aujourd'hui</p>
              </div>
            ) : (
...
const Section: React.FC<{ title: string; icon: React.ReactNode; children: React.ReactNode }> = ({ title, icon, children }) => (
  <div>
    <div className="flex items-center gap-2 mb-3">
      <div className="w-6 h-6 bg-blue-50 rounded-lg flex items-center justify-center text-blue-500">{icon}</div>
